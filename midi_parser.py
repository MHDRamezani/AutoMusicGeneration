...
def parseMidi(midi_file):
    ...
    for track in midi_data.tracks:
        ...
        for event in track:
            ...
            if str(event.type) == 'note_on' and event.velocity > 0 and event.note in range(lowest_note,
                                                                                           highest_note + 1):
                # note is played
            ...
            elif (str(event.type) == 'note_off' or str(event.type) == 'note_on') and event.note in range(lowest_note,
                                                                                                         highest_note + 1):
                # note is not played
        ...
        for i in range(len(delta)):
            if delta[i] == 1 or delta[i] == -1:  # play/stop note
                event = Message(note_events[delta[i] > 0],
                                time=int(scale * (current_index - last_index) * ticks_per_time), velocity=velocity,
                                note=(lowest_note + i))
                track.append(event)
